...
final class DocumentContextManager {
  ...
  String buildGroundedSystemPrompt({
    ...
    final buffer = StringBuffer()
      ..writeln('YOU ARE AN ENTERPRISE AI WRITING ASSISTANT AND SAAS DOCUMENT ENGINE.')
      ..writeln('Produce output formatted cleanly as semantic Markdown headings (#, ##, ###), lists (- or 1.), callouts (💡), quotes (>), and code snippets (```).')
      ..writeln();
    ...
    if (project != null) {
      ...
      if (project.context != null) {
        buffer.writeln('- System Instructions: ${project.context!.systemInstructions}');
        if (project.context!.targetAudience != null) {
          buffer.writeln('- Target Audience: ${project.context!.targetAudience}');
        }
        if (project.context!.architectureNotes != null) {
          buffer.writeln('- Architecture Guidelines: ${project.context!.architectureNotes}');
        }
      }

      if (project.knowledgeFiles.isNotEmpty) {
        buffer.writeln('- Active Knowledge Files (${project.knowledgeFiles.length}):');
